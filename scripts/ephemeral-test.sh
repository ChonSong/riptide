...
cleanup() {
    # Guard against double invocation from EXIT trap + signal handlers
    [[ "$CLEANING_UP" == "true" ]] && return
    CLEANING_UP=true
    ...
    # Remove any Hermes probe jobs that were created
    if [[ ${#PROBE_JOB_IDS[@]} -gt 0 ]]; then
        echo "Removing Hermes probe jobs..."
...
if [[ "$BRANCH" == refs/* ]]; then
    ...
    if ! git fetch origin "$REF" 2>&1; then
...
# Create temporary detached worktree (portable mktemp)
WORKTREE_DIR=$(mktemp -d -t riptide-worktree-XXXXXX 2>/dev/null || mktemp -d riptide-worktree-XXXXXX)
...
if ! docker network create "$NETWORK_NAME" 2>/dev/null; then
    # Network may already exist from a previous run — verify it's ours
    if ! docker network ls --format '{{.Name}}' | grep -q "^${NETWORK_NAME}$"; then
...
# Health check: 401 = webhook running, signature verification active
# (expected for unsigned test payload — confirms endpoint is live)
...
if [[ "$DO_PROBE" == "true" ]]; then
    ...
    if ! command -v hermes &>/dev/null; then
        ...
        LONGCAT_ID=$(echo "$LONGCAT_OUTPUT" | grep -oE '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}' | head -1 || true)
        ...
        CUSTOM_ID=$(echo "$CUSTOM_OUTPUT" | grep -oE '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}' | head -1 || true)
        if [[ -n "$CUSTOM_ID" ]]; then
