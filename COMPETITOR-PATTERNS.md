...
CodeRabbit integrates **static analysis before LLM review**. The quote below documents that static-analysis tools are loaded per changed files and their output forwarded to the LLM — it does not claim all linters run on every review:
...
Separately, CodeRabbit provides a catalog of **40+ built-in linters** covering ESLint, Pylint, Golint, RuboCop, and more (the appropriate subset is selected per repo/config):
...
|---------|--------|----------|------------------------|
| **Static analysis → LLM handoff** | CodeRabbit | Static analysis runs before the LLM; CodeRabbit ships 40+ built-in linters (not all run per review — appropriate tools are selected per changed files) | **Adopt.** Extend `diff_analyzer.py` to emit structured findings, then pass to LLM for enrichment. The existing security/complexity/error-handling patterns are the start of this pipeline. |
...
| **Edit same comment on review cycle** | Greptile | "Greptile may edit the same general PR comment on each review cycle" | **Adopt.** This is the exact two-tier flow. GitHub's `PATCH /repos/{owner}/{repo}/issues/comments/{comment_id}` (PR top-level comment) enables it; `POST /repos/{owner}/{repo}/issues/{pr}/comments` creates the comment. |
...
| Pattern | Source | Evidence | Adoption Recommendation |
|---------|--------|----------|------------------------|
| **Progressive comment refinement** | Greptile | Edits same comment on each review cycle | **Adopt.** Post "🔍 Riptide reviewing..." immediately. **PATCH the shared comment after each completed review pass** (deterministic → security → complexity → arch → test). Final version is the enriched review. Review-cycle sequencing: one comment per PR revision; the cycle counter in the footer increments on each new push revision. |
...
- Run test status check **if available** — source is GitHub Checks API (WS-3 scope); if unavailable, set `test_status: {"available": False, "status": None}` and continue. The pass must stay executable without test status.
...
- PATCH the Phase 2 comment **after each completed pass** (per-pass updates — see Pillar 4); the final PATCH is the complete enriched review
- Structure:
  ```text
...
- **Confidence threshold:** LLM-enriched findings below `confidence < 0.6` are filtered out. **Deterministic findings (diff_analyzer, graphify) carry no confidence value — they are retained by default**; they are only excluded by the dedup rules below. This keeps deterministic signals authoritative and lets confidence filter only LLM-generated enrichment.
...
- The exact number of CodeRabbit's "7-8 models" and their individual roles is from the Substack analysis ([theaiengineer.substack.com — How CodeRabbit Actually Works](https://theaiengineer.substack.com/p/how-coderabbit-actually-works)); CodeRabbit's own docs don't break down the ensemble composition. **[Inference]**
