...
class TestIsFixable:
    ...
    def test_none_name_is_non_fixable(self):
        """None name should not crash, should default to non-fixable."""
        assert self.verifier._is_fixable({"name": None}) is False

    def test_missing_name_is_non_fixable(self):
        """Missing name key should not crash, should default to non-fixable."""
        assert self.verifier._is_fixable({}) is False

    def test_empty_name_is_non_fixable(self):
        """Empty string name should not crash, should default to non-fixable."""
        assert self.verifier._is_fixable({"name": ""}) is False
