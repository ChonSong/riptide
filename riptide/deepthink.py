...
def _spawn_deepthink(
    ...
    prompt = (
        ...
        f"import subprocess\n"
        f"\n"
        f"# Ensure cwd is riptide so the import works\n"
        f"subprocess.run(['python3', '-c', 'from riptide.grafiphy.excalidraw_renderer import render_review'],\n"
        f"              cwd='~/workspace/riptide')\n"
        f"\n"
        f"findings = [\n"
        ...
        f"```\n\n"
        f"The renderer automatically creates sections: scope, code chunks with WHY, "
        f"god nodes, communities, connections, findings with severity colors, and legend.\n"
